...
action :create do
  ...
  if !::File.exists?(node['mysqlwrapper']['replication_sql']) then
    ...
    ::Chef::Log.info("mysqldump -uroot -p#{@new_resource.password} --all-databases --master-data=1 > #{@new_resource.sqlfile}")
    %x[mysqldump -uroot -p#{@new_resource.password} --all-databases --master-data=1 > #{@new_resource.sqlfile}]
